...
function countTasks(tasks) {
    const counts = { failed: 0, blocked: 0, pending: 0, inProgress: 0 };
    for (const task of tasks) {
        if (task.isFailed)
            ...
            counts.inProgress++; // scan yields only these 4 states; IN_PROGRESS is the fallback
    }
    return counts;
}
function sortedTasks(all) {
...
export async function formatOverview(tasksDir, tick) {
...
export async function printOverview(tasksDir, tick) {
    console.log(await formatOverview(tasksDir, tick));
}
export async function formatRunSummary(tasksDir, ticks) {
    const tdb = TaskDb.open(join(tasksDir, 'state.db'));
    try {
        const tasks = sortedTasks(TaskState.scan(tdb, tasksDir));
        const counts = countTasks(tasks);
...
export async function printRunSummary(tasksDir, ticks) {
    for (const line of await formatRunSummary(tasksDir, ticks))
        console.log(line);
}
