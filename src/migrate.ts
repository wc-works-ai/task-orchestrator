...
const TASK_DIR = /^T(\d+)-(.+)$/;
const PASSTHROUGH = new Set<TaskStatus>(['PENDING', 'FAILED', 'BLOCKED', 'CONVERGED']);
...
export function migrateShards(tdb: TaskDb, tasksRoot: string): number {
  ...
  for (const shard of SHARDS) {
    ...
    for (const entry of readdirSync(shardDir)) {
      ...
      if (!m) continue; // not a task dir (ignores state.db*, .staging*, etc.)
      try {
        if (importOne(tdb, tasksRoot, shard, entry, Number(m[1]), m[2]!, maxFailures)) imported++;
      } catch (error) {
...
function importOne(
  ...
  if (tdb.getByNumber(taskNumber) !== undefined) return false; // already imported
...
function mapStatus(raw: string | null): TaskStatus {
  const s = (raw ?? '').trim();
  if (s.startsWith('IN_PROGRESS')) return 'FAILED'; // claim is not migrated
  if (PASSTHROUGH.has(s as TaskStatus)) return s as TaskStatus;
  return 'PENDING';
}

function readNonNegInt(path: string): number {
...
function readDeps(path: string): number[] {
  const raw = readText(path);
  if (raw === null) return [];
  const deps: number[] = [];
...
function readBranch(path: string): string | null {
  const raw = readText(path);
