...
export class PiAgent implements CodingAgent {
  ...
  static #appendAgentLog(agentLog: AgentLog, text: string, state: RunState, action: string): void {
    /* v8 ignore next -- after the first log-write failure we intentionally stop retrying */
    if (state.logWriteFailed) return;
    try {
  ...
  static #writeAutoresearchScript(taskDirectory: string): void {
